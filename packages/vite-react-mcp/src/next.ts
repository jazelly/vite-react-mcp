...
export interface ReactMcpNextOptions {
...
const NEXT_BRIDGE_STATE_KEY = Symbol.for('vite-react-mcp.nextBridgeServers');
...
const writeNextClientEntry = (
  ...
  const generatedDirectory = path.join(rootDir, '.next/vite-react-mcp');
...
const startNextBridgeServer = (options: ReactMcpNextOptions): string | null => {
  ...
  httpServer.once('error', (error) => {
    ...
    console.warn(
      `[vite-react-mcp] Next bridge server failed on ${host}:${port}:`,
  ...
  httpServer.listen(port, host, () => {
    ...
    console.info(
      `[vite-react-mcp] Next MCP bridge listening on http://${host}:${port}`,
    );
  });
...
export const withReactMcpNext = (
  nextConfig: NextConfig = {},
  options: ReactMcpNextOptions = {},
): NextConfig => {
  return {
    ...nextConfig,
