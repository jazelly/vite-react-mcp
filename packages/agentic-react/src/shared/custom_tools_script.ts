    ...
    .map((tool) => {
      ...
      if (typeof tool.clientFunction === 'function') {
        ...
        if (functionSource.includes('[native code]')) {
          return `
            console.error('[agentic-react] Unable to register custom tool', ${safeToolName}, ': native functions are not supported as clientFunction');
          `;
        }
        return `
      ...
      return `
        import(${safeSpecifier})
          ...
          .catch((error) => {
            console.error('[agentic-react] Failed to load custom tool', ${safeToolName}, ':', error);
          });
      `;
    })
    .join('\n');
