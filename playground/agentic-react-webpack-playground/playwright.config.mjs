...
const portStatePath = path.join(
  os.tmpdir(),
  `agentic-react-webpack-playground-${portStateKey}.json`,
);
const portLockPath = path.join(
  os.tmpdir(),
  `agentic-react-webpack-playground-${portStateKey}.lock`,
);
...
const allocatePlaywrightPort = async () => {
  ...
  try {
    const port = await findAvailablePort(
      Number(process.env.AGENTIC_REACT_WEBPACK_PLAYGROUND_PORT || 51425),
    );
    writePortState(port);
    return port;
  } finally {
...
process.env.AGENTIC_REACT_WEBPACK_PLAYGROUND_PORT = String(playgroundPort);
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...process.env,
      AGENTIC_REACT_WEBPACK_PLAYGROUND_PORT: String(playgroundPort),
    },
    url: playgroundUrl,
    timeout: 180000,
