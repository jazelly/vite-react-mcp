...
const portStatePath = path.join(
  os.tmpdir(),
  `agentic-react-next-playground-${portStateKey}.json`,
);
const portLockPath = path.join(
  os.tmpdir(),
  `agentic-react-next-playground-${portStateKey}.lock`,
);
...
const findAvailablePort = async (preferredPort) => {
  ...
  throw new Error(
    `Could not find an available Next bridge port starting at ${preferredPort}.`,
  );
};
...
const waitForPortState = async () => {
  ...
  throw new Error('Timed out waiting for Next bridge port allocation.');
};

const allocatePlaywrightBridgePort = async () => {
  ...
  try {
    const port = await findAvailablePort(
      Number(process.env.AGENTIC_REACT_NEXT_BRIDGE_PORT || 51426),
    );
    writePortState(port);
    return port;
  } finally {
...
const bridgePort = await allocatePlaywrightBridgePort();
process.env.AGENTIC_REACT_NEXT_BRIDGE_PORT = String(bridgePort);
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...process.env,
      AGENTIC_REACT_NEXT_BRIDGE_PORT: String(bridgePort),
    },
